...
在 `<PhotoProvider />` 上添加参数 `loop` 可以更改循环预览数量。设为 `boolean` 启用/关闭，设为 `number` 类型则达到具体数量以开启循环预览，默认值为 `3` 张：
...
`pullClosable` 是否可以通过下拉关闭画廊，默认 `true`，`maskClosable` 是否可以点击蒙版关闭画廊，默认 `true`。
