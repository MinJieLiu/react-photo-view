...
- When `mode` is `drag` mode, the minimum scaling can reach `0.1`, and `1` is the original size

- When `mode` is `slide` mode, the zoom minimum can reach `1`, and `1` is the current view size
...
<PhotoProvider
  toolbarRender={({ onScale, scale }) => {
    return (
      <>
        <svg tw="box-border p-2.5 fill-white opacity-75 cursor-pointer transition-opacity ease-linear delay-200 hover:opacity-100" onClick={() => onScale(scale + 1)} />
        <svg tw="box-border p-2.5 fill-white opacity-75 cursor-pointer transition-opacity ease-linear delay-200 hover:opacity-100" onClick={() => onScale(scale - 1)} />
      </>
    );
  }}
...
<PhotoProvider
  toolbarRender={({ rotate, onRotate }) => {
    return <svg tw="box-border p-2.5 fill-white opacity-75 cursor-pointer transition-opacity ease-linear delay-200 hover:opacity-100" onClick={() => onRotate(rotate + 90)} />;
  }}
/>
```
